...
| Error conditions (observed) | Missing/empty `OWM_API_KEY`: the server does **not** return an MCP error — it returns a degenerate all-zeros response with an empty city name ("Current weather for :", all values 0, no forecast entries). Our parser rejects it (`NO_FORECAST_FOR_DATE`) and the day is assessed conservatively as weather-unknown. Invalid city: tool error text "city not found". Network failure: tool error "context deadline exceeded". |
