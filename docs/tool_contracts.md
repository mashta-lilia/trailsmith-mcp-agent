...
| Output schema | `{status: "ok"\|"invalid", normalized_itinerary: NormalizedItinerary, violations: [{code, severity, day, message}], applied_caps: {fitness, max_km, max_ascent_m}}`. `NormalizedItinerary` is an explicitly published model, **not** a free-form object: it is the exact input type of `estimate_logistics`, so the two tools share one schema and the output of tool 1 can be passed straight into tool 4. |
...
| Input schema | `segments: [string]` (required, 1–20 items matching `^CH-\d{3}$` — the same constraint `validate_itinerary` applies). `weather: {temp_min_c: -40..45, temp_max_c: -40..45, precip_mm: 0..500, wind_ms: 0..60, thunderstorm: bool}` — required when `weather_known` is true, **omitted otherwise**; unknown fields are rejected and `temp_min_c` must not exceed `temp_max_c`. `weather_known: bool` (default true). |
...
| Purpose | Produce a provisioning and daylight-safety sheet for an accepted itinerary. Called once at the end of the workflow. |
| Model-facing description | "Estimate logistics for an accepted itinerary: per-day hiking time (Naismith's rule), daylight margin computed from solar geometry at the route's latitude, water-source availability, and party-scaled food and water requirements. Call this only after the itinerary has been validated; pass the normalized_itinerary returned by validate_itinerary." |
| Input schema | `normalized_itinerary: NormalizedItinerary` — the object returned by `validate_itinerary` (1–7 days; each day `date` strict ISO, `segments` 1–20 `^CH-\d{3}$`, `total_km` 0..200, `total_ascent_m` 0..20000, `ends_at_shelter` bool; extra normalized keys such as `start_node` are accepted and used). `party: {fitness, size, has_tent}` — `size` drives provisioning; `fitness` and `has_tent` are part of the shared `Party` model and unused here. |
| Output schema | `{days: [{day, date, hiking_hours, daylight_hours, daylight_margin_hours, water_sources, ends_at_shelter}], food_days, party_size, food_kg, water_carry_litres, total_hiking_hours, water_warnings: [string], daylight_warnings: [string]}` |
| Computation | **Hiking time** — Naismith's rule: `total_km/4 + total_ascent_m/600`. `total_km` and `total_ascent_m` are **recomputed from `segments`**, never trusted from the input, so the two halves of a day sheet cannot describe different hikes. **Daylight** — computed from solar declination (Cooper's equation) at the latitude of the day's start node, taken from the dataset's own coordinates: `2·arccos(−tan φ · tan δ)/15`. This varies within a month (1 Aug = 15.0 h, 31 Aug = 13.1 h at 48°N) and with latitude, and it is what makes `daylight_margin_hours` meaningful. A margin below 1 h raises a `daylight_warnings` entry rather than silently going negative. **Water** — `water_sources` counts the day's `river_crossings` (a crossing is a refill point); a waterless day over 10 km raises a `water_warnings` entry; `water_carry_litres` assumes 1 L per person per 4 h on days with no crossing. **Food** — 0.7 kg dry food per person-day. |
| Error conditions | `UNKNOWN_SEGMENT`; `DISCONNECTED_DAY` (a day's segments do not chain — run `validate_itinerary` first); schema rejection for a malformed date, out-of-range totals, or >7 days. This tool cannot see `status`, so it will cost an itinerary that `validate_itinerary` rejected on a hard violation; the caller owns that gate. |
| Side effects | None (read-only over the local dataset). |
| Example | Input: the 2-day August plan above (6.6 km/790 m, 9.3 km/230 m), party of 4 → day 1 `hiking_hours: 3.0, daylight_hours: 13.8, daylight_margin_hours: 10.8, water_sources: 0`; totals `food_days: 2, food_kg: 5.6, water_carry_litres: 3.0, total_hiking_hours: 5.7`, no warnings. |
